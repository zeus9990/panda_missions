...
class AdminCog(commands.Cog):
    ...
    async def award_xp(self, interaction: discord.Interaction, member: discord.Member, amount: int, reason: str = "Good behaviour & community vibes"):
        ...
        try:
            ...
            logger.error("Award XP command failed — mod: %s target: %s: %s", interaction.user.id, member.id, e, exc_info=True)
            await interaction.followup.send("❌ Something went wrong while awarding XP.", ephemeral=True)
    ...
    async def verify_mission(self, interaction: discord.Interaction, member: discord.Member, mission: app_commands.Choice[str]):
        ...
        try:
            ...
            logger.error("Verify mission failed — mod: %s target: %s mission: %s: %s", interaction.user.id, member.id, mission_key, e, exc_info=True)
            await interaction.followup.send("❌ Something went wrong while verifying the mission.", ephemeral=True)
    ...
    async def user_profile_admin(self, interaction: discord.Interaction, member: discord.Member):
        ...
        try:
            ...
            logger.error("Admin Profile command failed for user %s: %s", interaction.user.id, e, exc_info=True)
            await interaction.followup.send("❌ Something went wrong loading the user profile.")
